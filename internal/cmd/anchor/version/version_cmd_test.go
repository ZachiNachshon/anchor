...
func Test_InstallCommandShould(t *testing.T) {
	tests := []harness.TestsHarness{
		{
			Name: "start version action successfully",
			Func: StartVersionActionSuccessfully,
		},
		{
			Name: "fail version action",
			Func: FailVersionAction,
		},
		{
	...
	with.Context(func(ctx common.Context) {
		with.Logging(ctx, t, func(logger logger.Logger) {
			with.Config(ctx, config.GetDefaultTestConfigText(), func(config *config.AnchorConfig) {
				...
				assert.Equal(t, 1, callCount, "expected action to be called exactly once. name: version")
...
var AddItselfToParentCommand = func(t *testing.T) {
	with.Context(func(ctx common.Context) {
		...
		assert.Equal(t, "version", cmds[0].Use)
	})
}
